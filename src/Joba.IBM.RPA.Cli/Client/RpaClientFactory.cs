...
namespace Joba.IBM.RPA.Cli
{
    public class RpaClientFactory : IRpaClientFactory
    {
        ...
        public RpaClientFactory(ILogger logger, IConsole console)
        {
            this.logger = logger;
            this.console = console;
        }

        public IRpaClient CreateFromAddress(Uri address) => new RpaClient(HttpRpaFactory.Create(logger, address));
        ...
        IRpaClient IRpaClientFactory.CreateFromPackageSource(PackageSource source)
        {
            ...
            var client = HttpRpaFactory.Create(logger, source.Remote.Address, new RenewExpiredSession(sessionEnsurer));
        ...
        IRpaClient IRpaClientFactory.CreateFromEnvironment(Environment environment)
        {
            ...
            var client = HttpRpaFactory.Create(logger, environment.Remote.Address, new RenewExpiredSession(sessionEnsurer));
